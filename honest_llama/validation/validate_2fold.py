...
def main(): 
    ...
    parser.add_argument('--model_name', type=str, default='llama3_8B_instruct', choices=HF_NAMES.keys(), help='model name')
    parser.add_argument('--model_prefix', type=str, default='', help='prefix to model name')
    parser.add_argument('--dataset_name', type=str, default='tqa_mc2', help='feature bank for training probes')
    parser.add_argument('--activations_dataset', type=str, default='tqa_gen', help='feature bank for calculating std along direction')
    ...
    df = pd.read_csv('/data/TAP/wangyujing/honest_llama/TruthfulQA/data/v1/TruthfulQA.csv')
    ...
    # print(len(list(df["Question"])))
    assert list(dataset['question']) == list(df["Question"])
    
    # get two folds using numpy
    fold_idxs = np.array_split(np.arange(64), args.num_fold)  #len(df)
